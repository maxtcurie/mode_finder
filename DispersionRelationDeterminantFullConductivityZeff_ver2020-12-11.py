...
-1.83379, -2.0739]]])
    ...
    A[0,0] = A[0,0] - 1/del_x**2*(1-ky*del_x)*BC
    A[num-3,num-3] = A[num-3,num-3] - 1/del_x**2*(1-ky*del_x)*BC
    A[num-2,num-2] = A[num-2,num-2] - 1/del_x**2*(1-ky*del_x)*BC
    A[2*num-5,2*num-5] =  A[2*num-5,2*num-5] - 1/del_x**2*(1-ky*del_x)*BC
    return A
...
#    delx=0.02
    ...
    while judge == 0:
        ...
        judge=int(input("Is this the correct mode with even parity?: 0 signifies no. 1 signifies yes: "))
...
def ParameterScan():
    ...
    if paramindex==5:
        ...
        w0Array=np.zeros(len(arrayParam),dtype=complex)
        for i in range(0,len(arrayParam)):
            w0=w_finder(xmax,delx,w0,nu,Zeff,eta,shat,beta,ky,ModIndex,arrayParam,xstar,0)
            w0Array[i]=w0
...
def Dispersion(nu,Zeff,eta,shat,beta,ky,ModIndex,mu,xstar):
  #Fit Parameters
    ...
    delx=0.050
    xmax=20
    wguess=3.0544238768638666+0.36255604477883j
    w0=w_finder(xmax,delx,wguess,nu,Zeff,eta,shat,beta,ky,ModIndex,mu,xstar)
    print(nu,Zeff,eta,shat,beta,ky,ModIndex,mu,xstar)
    print(w0)
...
#VectorFinder(3.4,2.35,1.41,0.011619,0.001984,0.025464,1,0.0,7.551903)
